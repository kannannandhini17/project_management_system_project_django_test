...
class TaskRegistrationForm(forms.ModelForm):
    project = forms.ModelChoiceField(queryset=Project.objects.all()) #QuerySets allow you to read the data from the database, filter it and order it.
    assign = forms.ModelMultipleChoiceField(queryset=User.objects.all()) #ORM-enables you to interact with your database
